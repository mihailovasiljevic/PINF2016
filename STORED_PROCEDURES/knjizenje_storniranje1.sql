 ...
 DECLARE
  ...
  IF @STORNO = 0
   BEGIN
	  ...
	  IF @MAG_ID IS NULL
	  BEGIN
		RAISERROR('Morate odabrati magacin!',11,2)
		RETURN
	  END

	  IF @MAG_MAG_ID IS NULL AND @PROM_DOK_VRST='M'
	  BEGIN
		RAISERROR('Morate odabrati magacin!',11,2)
		RETURN
	  END
...
-- DEKLARACIJA PODATAKA BITNIH ZA KURSOR --
 DECLARE
 ...
 IF @BROJ_TRANSAKCIJA = 0
  BEGIN TRANSACTION  
     -- DEKLARACIJA KURSORA --
  	 DECLARE 
	  cursPROKNJIZI 
	 CURSOR FOR SELECT STAV_PROM_DOK_ID,ROBA_ID,STAV_PROM_DOK_KOL,STAV_PROM_DOK_CEN,STAV_PROM_DOK_VRED
	 FROM STAVKA_PROMETNOG_DOKUMENTA
	 WHERE PROM_DOK_ID=@PROM_DOK_ID
	 --! DEKLARACIJA KURSORA --
	 -- OTVARANJE KURSORA --
	 OPEN cursPROKNJIZI
	 -- PREUZIMANJE SLEDECE VREDNOSTI --
	 FETCH NEXT FROM cursPROKNJIZI INTO @STAV_PROM_DOK_ID,@ROBA_ID,@STAV_PROM_DOK_KOL,@STAV_PROM_DOK_CEN,@STAV_PROM_DOK_VRED
	  WHILE @@FETCH_STATUS = 0
	   BEGIN

	    -- AKO JE KOLICINA U STAVCI MANJA OD 0 OBAVESTI KORISNIKA O TOME --
		IF @STAV_PROM_DOK_KOL < 0
		 ...
		 --  DEKLARACIJA VREDNOSTI ZA PRVU ROBNU KARTICU --
		 DECLARE			 
		 ...
		 IF @PROM_DOK_VRST = 'M'
			BEGIN
			 DECLARE			 
				@UKUPNA_KOLICINA2 numeric(14,4),
				@UKUPNA_VREDNOST2 numeric(14,4),
				@PROMET_ULAZA_KOLICINSKI2 numeric(14,4),
				@PROMET_ULAZA_VREDNOSNO2 numeric(14,4),
				@PROMET_IZLAZA_KOLICINSKI2 numeric(14,4),
				@PROMET_IZLAZA_VREDNOSNO2 numeric(14,4),
				@POCETNO_STANJE_KOLICINSKI2 numeric(14,4),
				@POCETNO_STANJE_VREDNOSNO2 numeric(14,4),
				@ROBNA_KARTICA_ID2 INT,
				@CENA2 numeric(14,4)	
			END
		 --!  DEKLARACIJA VREDNOSTI ZA PRVU ROBNU KARTICU --
		 	...
		 	-- PRVI MAGACIN --
			IF(SELECT COUNT(*) FROM ROBNA_KARTICA WHERE ROBA_ID = @ROBA_ID AND POSL_GOD_ID = @POSLOVNA_GODINA_ID AND MAG_ID = @MAG_ID) = 0
			BEGIN
			...
			END
		  --! PRVI MAGACIN --
		  -- AKO JE U PITANJU MEDJUMAGACINSKI PROMET, A U NEMA ROBNE KARTICU U NEKOM ILI OBA MAGACINA, NAPRAVI IH --
		 IF @PROM_DOK_VRST = 'M'
		  BEGIN
		  -- DRUGI MAGACIN --
			IF(SELECT COUNT(*) FROM ROBNA_KARTICA WHERE ROBA_ID = @ROBA_ID AND POSL_GOD_ID = @POSLOVNA_GODINA_ID AND MAG_ID = @MAG_MAG_ID) = 0
			BEGIN
				INSERT INTO ROBNA_KARTICA VALUES(@MAG_MAG_ID,@ROBA_ID, @POSLOVNA_GODINA_ID, 0,0,0,0,0,0,0,0,0)
		  ...
		  -- SELEKTUJ ROBU IZ PRVOG MAGACINA --
		  	SELECT @UKUPNA_KOLICINA = ROB_KART_UKKOL, 
				...
				@CENA = ROB_KART_CEBA
			FROM ROBNA_KARTICA 
			WHERE ROBA_ID = @ROBA_ID AND MAG_ID = @MAG_ID
		  --! SELEKTUJ ROBU IZ PRVOG MAGACINA --
		   --! SELEKTUJ ROBU IZ DRUGOG MAGACINA --
			IF @PROM_DOK_VRST = 'M'
			 BEGIN
		  		SELECT @UKUPNA_KOLICINA2 = ROB_KART_UKKOL, 
					@UKUPNA_VREDNOST2 = ROB_KART_UKVRED,
					@PROMET_ULAZA_KOLICINSKI2 = ROB_KART_PRUL_KOL,
					@PROMET_ULAZA_VREDNOSNO2 = ROB_KART_PRUL_VRED,
					@ROBNA_KARTICA_ID2 = ROB_KART_ID,
					@POCETNO_STANJE_KOLICINSKI2 = ROB_KART_POC_KOL,
					@POCETNO_STANJE_VREDNOSNO2 = ROB_KART_POC_VRED,
					@PROMET_IZLAZA_KOLICINSKI2 = ROB_KART_PRIZ_KOL,
					@PROMET_IZLAZA_VREDNOSNO2 = ROB_KART_PRIZ_VRED,
					@CENA2 = ROB_KART_CEBA
				FROM ROBNA_KARTICA 
				WHERE ROBA_ID = @ROBA_ID AND MAG_ID = @MAG_MAG_ID					
			 END
		 --!  SELEKTUJ ROBU IZ DRUGOG MAGACINA --
		  -- DA LI JE U PITANJU STORNIRANJE --
		  IF @STORNO = 1
		   BEGIN
			-- DA LI JE PRIMKA --
			 IF @PROM_DOK_VRST = 'P'
			 BEGIN
				 ...
				 -- DA LI UKUPNA KOLICINA PADA ISPOD 0 --
			...
			BEGIN
				...
				-- DA LI UKUPNA VREDNOST PADA ISPOD 0 --
			...
			-- DA LI JE MEDJUMAGACINSKI PROMET --

			 IF @PROM_DOK_VRST = 'M'
			  BEGIN
				 SET @PROMET_IZLAZA_KOLICINSKI = @PROMET_ULAZA_KOLICINSKI - @STAV_PROM_DOK_KOL
				 SET @PROMET_IZLAZA_VREDNOSNO = @PROMET_ULAZA_VREDNOSNO - @STAV_PROM_DOK_VRED
				 SET @UKUPNA_KOLICINA = @POCETNO_STANJE_KOLICINSKI + @PROMET_ULAZA_KOLICINSKI-@PROMET_IZLAZA_KOLICINSKI
				 SET @UKUPNA_VREDNOST = @POCETNO_STANJE_VREDNOSNO + @PROMET_ULAZA_VREDNOSNO-@PROMET_IZLAZA_KOLICINSKI
				 SET @CENA = (@UKUPNA_VREDNOST+@STAV_PROM_DOK_KOL*@STAV_PROM_DOK_CEN)/(@UKUPNA_KOLICINA+@STAV_PROM_DOK_KOL)
				 -- DA LI UKUPNA KOLICINA PADA ISPOD 0 --
				 IF(@UKUPNA_KOLICINA < 0)
					...
					END
				--! DA LI UKUPNA KOLICINA PADA ISPOD 0 --
				 SET @PROMET_ULAZA_KOLICINSKI2 = @PROMET_ULAZA_KOLICINSKI2 - @STAV_PROM_DOK_KOL
				 SET @PROMET_ULAZA_VREDNOSNO2 = @PROMET_ULAZA_VREDNOSNO2 - @STAV_PROM_DOK_VRED
				 SET @UKUPNA_KOLICINA2 = @POCETNO_STANJE_KOLICINSKI2 + @PROMET_ULAZA_KOLICINSKI2-@PROMET_IZLAZA_KOLICINSKI2
				 SET @UKUPNA_VREDNOST2 = @POCETNO_STANJE_VREDNOSNO2 + @PROMET_ULAZA_VREDNOSNO2-@PROMET_IZLAZA_KOLICINSKI2
				 SET @CENA2 = (@UKUPNA_VREDNOST2+@STAV_PROM_DOK_KOL*@STAV_PROM_DOK_CEN)/(@UKUPNA_KOLICINA2+@STAV_PROM_DOK_KOL)
				 -- DA LI UKUPNA KOLICINA PADA ISPOD 0 --
				 IF(@UKUPNA_KOLICINA2 < 0)
					BEGIN
					  SET @PORUKA = 'Ukupna kolicina ne sme pasti ispod nule '
		  ...
		  -- AKO NIJE STORNIRANJE RADI KNJIZENJE --
		  ELSE
			BEGIN
			-- DA LI JE PRIMKA --
			 IF @PROM_DOK_VRST = 'P'
			...
			--! AKO NIJE STORNIRANJE RADI KNJIZENJE --
			-- UPDATE ROBNIH KARTICA --
				
				-- UPDATE ROBNE KARTICE 1 --
					 ...
					 SET 
						...
						ROB_KART_CEBA = @CENA
					 WHERE 
						ROBA_ID = @ROBA_ID AND MAG_ID = @MAG_ID
					 IF @@ERROR <> 0
				...
				-- UPDATE ROBNE KARTICE 2 --
					IF @PROM_DOK_VRST = 'M'
					 BEGIN
					 	UPDATE ROBNA_KARTICA
						 SET 
							ROB_KART_UKKOL = @UKUPNA_KOLICINA2,
							ROB_KART_UKVRED = @UKUPNA_VREDNOST2,
							ROB_KART_PRUL_KOL = @PROMET_ULAZA_KOLICINSKI2,
							ROB_KART_PRUL_VRED = @PROMET_ULAZA_VREDNOSNO2,
							ROB_KART_PRIZ_KOL = @PROMET_IZLAZA_KOLICINSKI2,
							ROB_KART_PRIZ_VRED = @PROMET_IZLAZA_VREDNOSNO2,
							ROB_KART_CEBA = @CENA
						 WHERE 
							ROBA_ID = @ROBA_ID AND MAG_ID = @MAG_MAG_ID
						 IF @@ERROR <> 0
			...
			-- INSERT ANALITIKA --
				 ...
				 IF @PROM_DOK_VRST = 'M'
				  BEGIN
					 SELECT @BROJ_ANALITIKA2 = COUNT(*) 
					 FROM ANALITIKA_MAGACINSKE_KARTICE
					 WHERE ROB_KART_ID = @ROBNA_KARTICA_ID2

					 SET @BROJ_ANALITIKA2 = @BROJ_ANALITIKA2 + 1
				  END
				...
				-- INSERT ANALITIKE ZA ROBNU KARTICU 1 --
					...
					set @VRSTA1 = 'P'
					 ...
					 IF @STORNO = 0
					  BEGIN
					   ...
					   VALUES(@ROBNA_KARTICA_ID, @BROJ_ANALITIKA, @SMER1, @STAV_PROM_DOK_KOL,@STAV_PROM_DOK_CEN,@STAV_PROM_DOK_VRED,@VRSTA1)
					  END
					 ELSE
					  BEGIN
					   INSERT INTO ANALITIKA_MAGACINSKE_KARTICE 
					...
					set @VRSTA2 = 'M'
					 ...
					 IF @STORNO = 0
					  BEGIN
					   ...
					   VALUES(@ROBNA_KARTICA_ID, @BROJ_ANALITIKA, @SMER1, @STAV_PROM_DOK_KOL,@STAV_PROM_DOK_CEN,@STAV_PROM_DOK_VRED,@VRSTA1)
					   INSERT INTO ANALITIKA_MAGACINSKE_KARTICE 
					   VALUES(@ROBNA_KARTICA_ID2, @BROJ_ANALITIKA2, @SMER2, @STAV_PROM_DOK_KOL,@STAV_PROM_DOK_CEN,@STAV_PROM_DOK_VRED,@VRSTA2)
					  END
					 ELSE
					  BEGIN
					   INSERT INTO ANALITIKA_MAGACINSKE_KARTICE 
